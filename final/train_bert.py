...
label2idx = train_loader.iterator.dataset.label2idx
task2idx = train_loader.iterator.dataset.task2idx
idx2task = train_loader.iterator.dataset.idx2task
if os.path.exists(path):
    model = torch.load(path)
else:
    model = BertForNLI(config, bert_type, label2idx)
