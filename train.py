...
import time
import numpy as np
from model import direct_communication_network
...
model = direct_communication_network(pre_trained_embedding, embedding_dim, num_task, 64250, hidden_size, message_size)
model.to(DEVICE)
...
for e in range(num_epoch):
    ...
    for batch_id, (train_x, train_y, task_type) in enumerate(train_loader):
        ...
        # mask = train_x != 0
        # mask = mask.float()
        outputs = model(train_x, task_index)
        ...
        if batch_id % 50 == 0:
            ...
            print("Batch [{}/{}], loss: {:.3f}, accuracy: {:.3f}, Time elapsed: {}".format(
                batch_id+1, len(train_loader), cum_loss, corrects / total, after - before))
            sys.stdout.flush()
            cum_loss = 0
            before = after
            total = 0
    ...
    with torch.no_grad():
        ...
        for val_batch_id, (val_x, val_y, task_type) in enumerate(val_loader):
            ...
            # mask = val_x != 0
            # mask = mask.float()
            val_out = model(val_x, task_index)
