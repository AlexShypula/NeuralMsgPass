...
from modules.message_lstm import message_lstm_dc
...
class direct_communication_network(nn.Module):
    def __init__(self,
        ...
        super(direct_communication_network, self).__init__()

        pretrained_embedding = torch.tensor(pretrained_embedding, dtype=torch.float64)
        self.num_tasks = num_tasks
        self.vocab_size = len(pretrained_embedding)
    ...
    def forward(self, x, TASK):
        ...
        #h_task = torch.zeros(embeddings.size(0), embeddings.size(1), self.hidden_size)
        ...
        for t in range(embeddings.size(1)):
            ...
            var2 = self.aggregation_activation(var1)  # var 2 is B x H x Task
            ...
            a = task_specific_lstm.softmax(Si)  # a is B x Task x 1, softmax over Tasks
            # transpose aggregated_t so that dimensions (B x H x Task) -> (B x Task x H)
            norm_aggregated_t = torch.mul(a, aggregated_t.transpose(1,2))  # (B x Task x 1 ) * (B x Task x H) -> (B x Task x H)
            Rt = torch.sum(norm_aggregated_t, dim=1)  # (B x H)
            output, state_h, state_c = task_specific_lstm._step(embeddings[:,t,:], Rt, state_h, state_c)
            #outputs.append(output)

        out = task_specific_lstm.task_specific_output(output)
